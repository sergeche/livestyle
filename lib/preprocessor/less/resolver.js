...
define(function(require, exports, module) {
	...
	function getExtendFromProperty(node) {
	...
	function collectInnerExtend(node) {
		...
		for (var i = 0, il = node.children.length; i < il; i++) {
			if (innerExtend = getExtendFromProperty(node.children[i])) {
	...
	function removeExtendFromPath(path) {
		for (var i = 0, il = path.length; i < il; i++) {
			path[i] = path[i].replace(reExtend, '').trim();
		}
	...
	function hasExtend(list) {
		...
		for (var i = 0, il = list.length; i < il; i++) {
			...
			for (var j = 0, jl = children.length; j < jl; j++) {
				if (getExtendFromProperty(children[j])) {
					return true;
				}
			}
		}
