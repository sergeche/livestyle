...
define(function(require, exports, module) {
	...
	/**
	 * Extracts arguments from given name
	...
	function findCandidates(dfn, mixins) {
		var argsDfn = extractArgs(dfn);
		var name = normalizeName(argsDfn.name.trim());
		...
		var matches = candidates.filter(function(item) {
			return matchesArguments(item, argsDfn.args);
		});
	...
	/**
	 * Tri to retreive mixins definitions from given node
	...
	function captureMixinDefs(node, parent, out) {
		...
		mxNames.forEach(function(name) {
			...
			saveMixin(name, node, guardsDef.guards, out);

			node.children.forEach(function(child) {
				captureMixinDefs(child, name, out);
			});
		});
	...
	return  {
		...
		resolve: function(node, state) {
			...
			var argsDef = extractArgs(name);
			var args = argsDef.args;
			name = argsDef.name;
			console.log('handling "%s" (%s)', name, node.type);
			...
			candidates.forEach(function(m) {
				...
				args.forEach(function(a, i) {
					vars[a.name] = expression.safeEval(a.raw, state.variables, true);
				});

				var innerState = state.clone({variables: vars});
				if (!matchesGuard(m, innerState.variables)) {
					console.log('does not matches guard');
					return;
				}
