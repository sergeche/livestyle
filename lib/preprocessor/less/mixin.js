...
define(function(require, exports, module) {
	...
	function splitArgs(str) {
		...
		var add = function(arg) {
			...
			var a = {
				name: parts.length > 1 ? parts.shift() : null,
	...
	function populateInvocationArguments(args, state) {
		...
		args.forEach(function(a) {
			var value = expression.eval(a.value, state.variables, true);
			if (a.name) {
				named[a.name] = value;
			} else {
	...
	function bindArguments(mixin, invokeArgs, state) {
		...
		mixin.args.forEach(function(arg, i) {
			...
			if (arg.name in invokeArgs.named) {
				...
				value = expression.eval(arg.value, state.variables, true);
			}
	...
	return  {
		...
		resolve: function(node, state) {
			...
			var args = populateInvocationArguments(signature.args, state);
