...
define(function(require, exports, module) {
	...
	function getResolved(item) {
		console.log('resolving "%s" (%s)', item.cleanSelector(), !!item.extendedBy);
		...
		if (item.extendedBy) {
			...
			item.extendedBy.forEach(function(ex) {
				...
				extendWith.forEach(function(exSel) {
					_.each(ex.sel, function(sel) {
						console.log('In [%s] extend "%s" with "%s"', item.cleanSelector(), sel.sel, exSel);
						...
						if (!sel.all) {
							...
							result = simpleExtendNew(sel.rules, exSel, item);
						} else {
							result = partialExtendNew(sel.rules, exSel, item);
						}
			...
			if (extended.length) {
				console.log('extended', _.uniq(extended));
				var path = item.listItem.path;
				var curSels = cleanSelectorsLookup(item.listItem).concat(extended);
				path[path.length - 1] = _.uniq(curSels).join(', ');
			}
		...
		return selector.rules(_.last(item.listItem.path));
	}
	...
	return {
		...
		resolve: function(list) {
			...
			return extending.map(function(item) {
				var path = item.listItem.path;
				path[path.length - 1] = getResolved(item).join(', ');
				return {
					path: path,
					node: item.listItem.node
				}
			});
		...
		extendingSelectors: function(list) {
			...
			// mark list items as extended
			result.forEach(function(item) {
				item.toExtend.forEach(function(ex) {
					if (!ex.extendedBy) {
						ex.extendedBy = [];
					}

					ex.extendedBy.push(item);
				});
			});
