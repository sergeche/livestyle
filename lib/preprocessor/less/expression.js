	...
	return {
		...
		interpolate: function(str, ctx, options) {
			...
			tokens.forEach(function(token) {
				...
				var result = expr in ctx ? ctx[expr] : expr;
				// var result = expression.eval('@' + expr, ctx, true);
				
				// XXX do I really have to unquote value?
				// result = unquote(result);
				str = utils.replaceSubstring(str, result, token);
			});
		...
		eval: function(expr, ctx, safe) {
			...
			expr = this.interpolate(expr, ctx);
			return expression.eval(expr, ctx, false);
		},
