...
define(function(require, exports, module) {
	...
	return {
		resolve: function(node, state) {
			...
			var n = state.parent;
			while (n && n.name) {
				if (reMedia.test(n.name)) {
					parts.unshift(n.name.replace(reMedia, ''));
				}
				n = n.parent;
			}
			...
			var innerState = state.clone({parent: target});
			// var innerState = _.defaults({parent: target}, state);
			node.children.forEach(function(child) {
				state.transform(child, innerState);
			});
