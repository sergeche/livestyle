...
define(function(require, exports, module) {
	...
	var reIsVar = /^\$/;
	var reAllowedSection = /@media|@supports/;
	...
	function resolveMixinContent(node, state) {
		...
		console.log(state.mixinContent);
		state.mixinContent.forEach(function(child) {
			transform(child, state);
		});
	}

	function findParentNodeForSection(node) {
		while (node.parent) {
			if (reAllowedSection.test(node.name)) {
				break;
			}
			node = node.parent;
		}
	...
	function resolveSection(node, state) {
		...
		path.push(expression.interpolate(node.name(), state.variables));

		out.name = nesting.resolvedNameForPath(path);

		// sections must be added on top level, except media queries
		// TODO check for media query support
		findParentNodeForSection(state.parent).addChild(out);
		state = _.defaults({parent: out}, state);
		node.children.forEach(function(child) {
			transform(child, state);
		});
	}
	...
	function transform(node, state) {
		...
		if (node.type == 'section') {
			return resolveSection(node, state);
		}
