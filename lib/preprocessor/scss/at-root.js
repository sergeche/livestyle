	...
	return {
		...
		resolve: function(node, state) {
			...
			if (atRoot.name) {
				...
				section.resolve(node, state.clone({parent: top}), atRoot.name);
				return true;
			}
			...
			var anonState = state.clone({parent: anonParent});

			return state.next(node, state.clone({parent: top}), function(child, state) {
				if (child.type == 'property') {
					...
					section.resolve(child, state.clone({parent: root}), nesting.resolvedNameForPath(path));
				} else {
