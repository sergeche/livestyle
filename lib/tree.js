define(['lodash', 'cssParser', 'range'], function(_, cssParser, range) {
	...
	function CSSNode(nameRange, valueRange, source, hints) {
		...
		this._hints = hints;
		// if (hints.name) {
		// 	this._name = parseSelectors(hints.name).join(', ');
		// 	// console.log(parseSelectors(hints.name).join(', '));
		// }
	}
	...
	function consumeSection(iter) {
		...
		while (token = iter.peek()) {
			if (token.type === '@') {
				...
				if (iter.peek().type === ';') {
					// at-rule end
					iter.ctx.addChild(new CSSNode(iter.curRange(true), range.create(iter.peek().start, 0), iter.source, {name: curTokens()}));
					iter.next();
					skipFormatting(iter);
					iter.start = iter.pos;
					continue;
