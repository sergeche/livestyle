define(['lodash', 'cssParser', 'range'], function(_, cssParser, range) {
	...
	CSSNode.prototype = {
		...
		toFullSource: function() {
			return this.fullRange().substring(this.source()).trim();
		},
		...
		insert: function(subtree, pos) {
			...
			var startPos = tokens[targetToken].start, subtreeSourceLen = 0;
			...
			shiftPositions(subtree.all(), {
				charStart: 0,
				charOffset: targetToken ? tokens[targetToken - 1].end : 0,
				tokenStart: 0,
			...
			shiftPositions(siblings, {
				...
				charStart: tokens[targetToken].start,
				charOffset: _.last(subtreeTokens).end
			});
		...
		replace: function(subtree) {
			var ix = this.index();
			this.remove();
			return this.parent.insert(subtree, ix);
		}
	};
	...
	function shiftPositions(items, options) {
		_.each(items, function(item) {
			// update tokens
			var h = item._hints;
			if (h.startToken >= options.tokenStart) {
				h.startToken += options.tokenOffset;
			}
			if (h.endToken >= options.tokenStart) {
				h.endToken += options.tokenOffset;
			}

			if (h.name) {
				if (h.name[0] >= options.tokenStart) {
					h.name[0] += options.tokenOffset;
				}
				if (h.name[1] >= options.tokenStart) {
					h.name[1] += options.tokenOffset;
				}
			}
