define(['lodash', 'cssParser', 'range'], function(_, cssParser, range) {
	...
	function makeSel(r, source) {
		return r.substring(source).trim().replace(reS, ' ').toLowerCase();
	};
	...
	function parseSelectors(source) {
		...
		while (iter.hasNext()) {
			token = iter.peek();
			if (token.type === ',') {
				selectors.push(makeSel(iter.curRange(true), source));
				iter.next();
				skipFormatting(iter);
				iter.start = iter.pos;
			} else {
		...
		selectors.push(makeSel(r, source));
	...
	function skipFormatting(iter) {
		while (iter.peek() && iter.peek().type in fTokens) {
			iter.next();
		}
	}
