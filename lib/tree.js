		...
		name: function(isLower) {
			...
			if (this._name === null) {
				...
				if (this.type === 'section') {
					this._name = cssSections.splitSelector(this._name).join(', ');
				}
			}
		...
		for (var j = 0, jl = properties.length, p, m; j < jl; j++) {
			...
			if (!p.value.length()) {
				...
				if (m = originalName.match(reAtRule)) {
					var value = originalName.substring(m[0].length);
					var end = '';
					var name = m[1];
					...
					node.addChild(new CSSNode(
						...
						range(p.name.start + m[0].length, value), 
						{
							type: 'at-rule',
	...
	return {
		...
		build: function(source) {
			...
			if (!sectionTree.children.length) {
				...
				consumeProperties(root, source.substring(lastSection.range.end));
			}
