...
define(function(require, exports, module) {
	...
	var lessCtx = require('./preprocessor/less/context');
	...
	function bestPartialMatch(cssTree, cssPath, patch, options) {
		...
		if (loc.rest) {
			...
			var rule = patchSection(loc.node.ref, patch, options);
	...
	function patchSection(section, patch, options) {
		...
		var style = options.style || learnStyle(section);
		...
		_.each(patch.properties, function(prop) {
			...
			console.log('rule prop', ruleProp);
			if (ruleProp) {
				value = prop.value;
				if (options.syntax == 'less') {
					console.log('patch as LESS');
					if (!ctx) {
						ctx = lessCtx.context(section, options);
					}
					resolved = lessCtx.patch(section.children[rule.indexOf(ruleProp)], value, ctx);
					if (resolved.safe || options.allowUnsafe) {
						value = resolved.value;
					}
				}
				ruleProp.value(value);
			} else {
	...
	return {
		...
		patch: function(source, patches, options) {
			...
			var sourceTree = sourceResolver.resolve(source);
			...
			_.each(patches, function(patch) {
				...
				if (!section) {
					...
					var rule = patchSection(section.ref, patch, options);
