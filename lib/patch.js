...
define(function(require, exports, module) {
	...
	function learnStyle(node) {
		...
		if (donor && donor.parent) {
			var donorSource = donor.toSource();
			style.selSep = donorSource.substring(donor.nameRange.end, donor.valueRange.start);
			cssDOM = cssEditTree.parse(donorSource);
		}
	 ...
	 * @return {CSSNode}
	 */
	function bestPartialMatch(cssTree, cssPath, patch, options) {
		options = options || {};
		var loc = locator.guessLocation(cssTree, cssPath);
		if (loc.node.parent) {
			var ctxName = loc.node.name(true);
		...
		if (loc.rest) {
			...
			var opt = _.extend({}, options, {style: style});

			// incomplete match, create new sections and inject it into tree
			// TODO try to implement it for preprocessors
			...
			if (diff.shouldCompareValues(lastSection[0]) && 'value' in patch) {
				...
				_value = patchSection(tree.build(_value).children[0], patch, opt);
			}
			...
			var rule = patchSection(loc.node, patch, options);
	...
	function patchSectionByValue(section, patch) {
		var ref = section.ref;
		var fullSource = ref.toFullSource();
		var value = ref.rawValue();
		var offset = ref.fullRange().start;
		var vStart = ref.valueRange.start - offset;
		var vEnd = ref.valueRange.end - offset;
	...
	/**
	 * Makes gived resolved section editable: adds `edit` 
	 * property that holds `cssEditTree` instance for section and
	 * each property
	 * @param  {ResolvedNode} section
	 * @param  {Object} options 
	 * @return {Array} List of section properties
	 */
	function makeEditable(section, options) {
		var style = options.style || learnStyle(section.ref);
		var lookup = {};
		var properties = section.properties();
		section.edit = cssEditTree.parse(section.ref.toFullSource(), {
		...
		properties.forEach(function(item) {
			...
			item.edit = section.edit.getAll(name)[lookup[name]];
		});
		return properties;
	}

	function patchSection(section, patch, options) {
		if ('value' in patch) {
			// updating full value of section, like `@import`
			return patchSectionByValue(section, patch);
		}

		var resolvedProperties = makeEditable(section, options);
		var lookup = {};
		resolvedProperties.forEach(function(property) {
			var name = property.name;
			if (!lookup[name]) {
				lookup[name] = [];
			}

			lookup[name].push(property);
		});

		// update properties
		patch.properties.forEach(function(item) {
			var property = resolvedProperties[item.index];
			if (!property || property.name != item.name) {
				property = _.last(lookup[item.name]);
			}

			if (property) {
				var patchedValue = sourceResolver.patch(property, item.value, options);
				property.edit.value(patchedValue);
			} else {
				section.edit.value(item.name, item.value);
			}
		});

		// remove properties
		patch.removed.forEach(function(item) {
			section.edit.remove(item.name);
		});

		return section.edit.source;
	}
	...
	return {
		...
		patch: function(source, patches, options) {
			options = _.defaults(options || {}, {syntax: 'css'});
			patches = this.condense(_.isArray(patches) ? patches : [patches]);
			var sourceTree = sourceResolver.resolve(source, options);

			_.each(patches, function(patch) {
				var cssPath = locator.parsePath(patch.path);
				var section = locator.locate(sourceTree, cssPath, options);
				...
				if (!section) {
					bestPartialMatch(sourceTree, cssPath, patch, options);
				} else {
					// XXX why do I have to replace rule source instead of value update?
					var rule = patchSection(section, patch, options);
					// update matched section with new source
					section.ref.replace(tree.build(rule));
				}
			});

			return sourceTree.ref.source();
		},
