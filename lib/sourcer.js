define(['lodash', 'tree', 'locator'], function(_, tree, locator) {
	...
	return {
		...
		/**
		 * Tries to find updated CSS property inside given
		 * CSS source for specified caret position.
		 * @param  {String} source CSS source
		 * @param  {Number} caret  Caret position inside source
		 * @return {Object}        Updated payload
		 */
		findUpdated: function(source, caret) {
			var rule = emmet.require('cssEditTree').parseFromPosition(source, caret, true);
			if (!rule) {
				return null;
			}
			...
			var prop = rule.itemFromPosition(caret, true);
			var values = {};
		...
		/**
		 * Patches CSS source with given data
		 * @param  {String} source CSS source
		 * @param  {Object} payload   Update payload
		 */
		patch: function(source, payload) {
			if (_.isString(payload)) {
				payload = JSON.parse(payload);
			}
			...
			var node = locator.locate(cssTree, cssPath);
			if (node) {
				upd = updateValue(node, payload.value);
			} else {
				var loc = locator.guessLocation(cssTree, cssPath);
				if (!loc.rest) {
					updateValue(loc.node, payload.value);
				} else {
					// Path partially matched, but we have to re-create
					// all sections
