define(['lodash', 'tree', 'locator'], function(_, tree, locator) {
	...
	function removePropertiesFromRule(rule, properties) {
		properties
			...
			.forEach(function(p) {
				...
				_.invoke(rule.getAll(p.name), 'remove');
			});
	}
	...
	return {
		...
		makePatch: function(source, pos) {
			...
			var sources = _.initial(arguments);
			var cssTree = _.isString(sources[0]) ? tree.build(sources[0]) : sources[0];
			...
			var propJSON = function(p) {
				return {
					name: p.name(),
					value: p.value(),
					index: rule.indexOf(p)
				};
			};

			var props = [], removed = null;
			if (prop) {
				props.push(propJSON(prop));
			} else if (sources.length > 1) {
				// we have another source,
				// try to compare their structures
				var upd = this.compareSections(sources[0], sources[1], pos);
				props = upd.added;
				removed = upd.removed;
			} else {
				props = rule.list().map(propJSON);
			}
		...
		compareSections: function(source1, source2, pos) {
			...
			var tested = [];
			var och = tree1.children;

			// find added properties
			var added = tree2.children.filter(function(p, i) {
				var ix = -1;
				_.find(och, function(c, i) {
					if (c.name() == p.name() && c.value() == p.value()) {
						ix = i;
						return true;
					}
				});
			...
			// find removed
			var removed = _.compact(och.map(function(p, i) {
				if (!_.include(tested, i)) {
					return {
						name: p.name(),
						value: p.value(),
						index: i
		 ...
		 * @return {Object}
		 */
		compareSections2: function(source1, source2, pos) {
			if (pos === void 0) {
			...
			var tested = [], added = [], updated = [], removed = [];
			var lookupIx = 0;

			var json = function(p, addIndex) {
				var out = {
					name: p.name(),
					value: p.value()
				};
				...
				if (addIndex) {
					out.index = _.indexOf(p.parent.children, p);
				}
