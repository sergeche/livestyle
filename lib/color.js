...
define(function(require, exports, module) {
	...
	return {
		...
		toHex: function(rgb) {
			return '#' + this.toArray(rgb).map(function (c) {
		...
		toDecimal: function(r, g, b) {
			if (arguments.length == 1) {
				if (Array.isArray(r)) {
					...
					r = r.replace(/^#/, '');
					if (r.length == 3) {
						r = r.charAt(0) + r.charAt(0)
							+ r.charAt(1) + r.charAt(1)
							+ r.charAt(2) + r.charAt(2);
					}
		...
		toARGB: function (rgb, a) {
			...
			return this.toHex([a * 255].concat(this.toArray(rgb)));
		}
	};
});