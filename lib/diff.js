...
define(function(require, exports, module) {
	...
	function getProperties(node, options) {
		...
		// 		return child.type === 'property';
		// 	});

		// 	return lessCtx.properties(node, options);
		// }

		return node.ref.properties();
	}
	...
	function sanitize(list) {
		...
		list.forEach(function(item) {
			['node', 'ref', 'parent'].forEach(function(name) {
				if (name in item) {
					delete item[name];
				}
			});
	...
	return {
		...
		diff: function(source1, source2, options) {
			...
			var l1 = sourceResolver.toList(t1, options);
			var l2 = sourceResolver.toList(t2, options);
			var out = [];
