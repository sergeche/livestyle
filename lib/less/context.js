...
define(function(require, exports, module) {
	...
	function findMixins(node) {
		var out = {};
		node.root().children.forEach(function(item) {
	 ...
	 * @return {Array}
	 */
	function resolveMixin(node, mixins) {
	...
	return module.exports = {
		...
		properties: function(node) {
			...
			node.properties().forEach(function(p) {
				...
				// property have no value: is it a mixin?
				if (!reMixin.test(p.name)) {
					return;
				}

				if (!mixins) {
					mixins = findMixins(node);
				}

				try {
					props = props.concat(resolveMixin(p.node, mixins));
				} catch (e) {
					logger.warn('Unable to resolve mixin ' + p.name, e);
				}
			});
		...
		context: function(node, vars) {
			// TODO cache variable scope for each section
			var ctx = {};
			// collect variables.
