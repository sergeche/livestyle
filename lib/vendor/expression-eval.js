...
define(function(require, exports, module) {
	...
	function Token(type_, index_, prio_, number_, unit_) {
		...
		this.valueOf = function () {
			...
			case TNUMBER:
				if (this.unit_ == '#') {
					return colorMod.toCSS(num(this), this.alpha);
				}
	...
	function colorToken(val, alpha) {
		...
		if (typeof val == 'string') {
			var m;
			if (m = val.match(reRGBA)) {
				val = colorMod.toDecimal(+m[1], +m[2], +m[3]);
				if (m[4]) {
					alpha = parseFloat(m[4])
				}
			} else if (m = val.match(reHexColor)) {
				val = colorMod.toDecimal(m[1]);
			}
		}

		var token = new Token(TNUMBER, 0, 0, clamp(val, maxColor), '#');
		token.alpha = clamp(typeof alpha == 'undefined' ? 1 : parseFloat(alpha), 1);
	...
	function evalArg(arg, hint, options) {
		...
		if (type == 'number') {
			...
			if (argType(hint) == 'color') {
				...
				token = colorToken(out.value, out.alpha);
				out.type = 'color';
				out.value = token.number_;
				out.unit = token.unit_;
			...
			if (typeof arg == 'string') {
				...
				token = colorToken(num(arg), arg.alpha);
			}
