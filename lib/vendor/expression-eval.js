...
define(function(require, exports, module) {
	...
	Expression.prototype = {
		evaluate: function (values) {
			...
			out = nstack[0];
			return out instanceof Token ? out.valueOf() : out;
		},
	...
	function evalArg(arg, hint, options) {
		options = options || {};
		var type = argType(arg), out;
		if (type == 'number') {
			out = {
				value: arg instanceof Token ? arg.number_ : arg,
				unit: arg instanceof Token ? arg.unit_ : ''
			};
			...
			if (argType(hint) == 'color') {
				...
				// number to color
				if (options.fixColor) {
					out.value = (out.value << 16) + (out.value << 8) + out.value;
				}
				out.unit = '#';
			}
		} else if (type == 'color') {
			if (typeof arg == 'string') {
				arg = evalColorString(arg.substr(1));
			} else {
				arg = arg.number_;
			}
	...
	function op(a, b, fn, options) {
		...
		if (_a.unit == '#' || _b.unit == '#') {
			out = Math.max(Math.min(out|0, maxColor), 0);
			out = out.toString(16);
			while (out.length < 6) {
				out = '0' + out;
			}
	...
	Parser.prototype = {
		parse: function (expr) {
			...
			var tokenstack = [];
			this.tmpprio = 0;
			var expected = (PRIMARY | COLOR | LPAREN | FUNCTION | SIGN);
			var noperators = 0;
			...
			while (this.pos < this.expression.length) {
				if (this.isOperator()) {
					if (this.isSign() && (expected & SIGN)) {
						...
						expected = (PRIMARY | COLOR | LPAREN | FUNCTION | SIGN);
					}
					...
					expected = (PRIMARY | COLOR | LPAREN | FUNCTION | SIGN | NULLARY_CALL);
				}
		...
		isColor: function () {
			...
			if (r) {
				if (str.length == 3 || str.length == 6) {
					this.tokennumber = evalColorString(str);
				} else {
		...
		isVar: function () {
			...
			for (var i = this.pos; i < this.expression.length; i++) {
				...
				if (c == '-') {
					fc = str.charAt(0);
					if (fc == '@' || fc == '$') {
						// LESS and SCSS variables allow dashed
						str += c;
						continue;
					}
				}
