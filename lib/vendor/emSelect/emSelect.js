(function (root, factory) {
	...
	Options.prototype.invoke = function(methodName) {
		if (typeof this.data[methodName] == 'function') {
			this.data[methodName].apply(this.data, sliceFn.call(arguments, 1));
		}
	};
	...
	function buildOptions(elem) {
		...
		for (var i = 0, il = sel.options.length, item, label, opt; i < il; i++) {
			...
			options.invoke('onItemCreated', item);
			optList.appendChild(item);
		}

		options.invoke('onItemListPopulated', optList);
		return optList;
	}
	...
	function hideOptions(elem) {
		...
		if (wData && wData.widget.classList.contains('em-select_active')) {
			...
			if (wData.popup) {
				...
				var p = wData.popup.parentNode;
				p.removeChild(wData.popup);
				wData.popup = null;
			}
		}
	}
	...
	document.addEventListener('click', function(evt) {
		...
		if (optItem) {
			...
			widgets[wid].options.invoke('onItemClicked', optItem, wid, evt);
	...
	var factory = function(elem, options) {
		...
		console.log(typeof elem.selectedIndex, elem.selectedIndex);
