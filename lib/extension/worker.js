	...
	onmessage = function(evt) {
		...
		switch (data.action) {
			...
			case 'diffExternalSources':
				try {
					success(data, {
						patches: diff.diff(data.source1, data.source2, options),
						source: data.source2
					});
				} catch (e) {
					error(data, e);
			...
			case 'patch':
				try {
					success(data, {
						...
						source: patch.patch(data.source, data.patches, options)
					});
				} catch (e) {
					error(data, e);
				}
			...
			case 'patchExternalSource':
				try {
					var out = patchAndDiff(data.source, data.patches, options);
					success(data, {
						patches: data.patches,
						content: out.content,
			...
			case 'diffUnsaved':
				...
				_.each(data.data.files, function(item) {
					try {
						out.push({
							file: item.file,
							patches: diff.diff(item.pristine, item.content, options)
						});
					} catch (e) {
