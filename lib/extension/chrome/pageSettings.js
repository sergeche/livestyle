 ...
 */
define(['lodash', 'patch', 'chrome/storage', 'chrome/utils', 'chrome/ports'], function(_, patch, storage, utils, ports) {
	var module;
	var inspectedTabs = {};
	...
	function _getPageSettingsByUrl(url, callback) {
		storage.get('pageSettings', function(data) {
			data = data || [];
	...
	function findPageSettingsForURL(settings, url) {
		...
		return guessPageSettingsForURL(settings, url);
	}

	/**
	 * Tries to guess best settings for given URL: locates the settings 
	 * for the domain name of given URL
	 * @param  {Array} settings Common settings storage
	 * @param  {String} url     Page URL
	 * @return {Number}         Index in `settings` array. Returns -1 if nothing found
	 */
	function guessPageSettingsForURL(settings, url) {
		var m = url.match(/^(https?:\/\/[^\/]+)/i);
		if (m) {
			var domain = m[1];
			for (var i = 0; i < settings.length; i++) {
				if (settings[i][0].indexOf(domain) === 0) {
					return i;
				}
			}
		}
	...
	chrome.runtime.onMessage.addListener(function(request, sender, sendResponse) {
		...
		if (request.action == 'tabNavigated') {
			...
			if (data.tabId in inspectedTabs) {
				savePatchInHistory(inspectedTabs[data.tabId].url, getUrlForHistory(data), data.patches);
			}
		}
	});
	...
	return module = {
		...
		save: function(data) {
			this.get(data, function(value, url, info) {
				...
				storage.set('pageSettings', fullData);
			});
		}
	};
});