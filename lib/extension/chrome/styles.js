...
define(['chrome/utils'], function(utils) {
	...
	return {
		 ...
		 * @param {String} url URL (or array of URLs) to remove. Can be either
		...
		remove: function(url, callback) {
			...
			var that = this;
			var _url = url.map(function(item) {
				return that.isUserFile(item) ? that.lookupBlobUrl(item) : item;
			});

			console.log('removing', _url);
			removeBlob(_url, function() {
				console.log('url removed');
				var blobs = url.map(function(item) {
					return that.lookupBlobUrl(item);
				});
				resetStylesCache();
				callback && callback(blobs, url);
			});
		},
