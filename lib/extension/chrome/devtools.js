...
require(['lodash', 'diff', 'patch', 'chrome/utils', 'chrome/styles'], function(_, diff, patch, utils, styles) {
	...
	function removeUserFiles(files) {
		files = _.isArray(files) ? files : [files];
		styles.remove(files, function(blobs, internals) {
			utils.getPageSettings(function(settings) {
				if (settings.userfiles) {
					console.log('removing', internals);
					internals.forEach(function(url) {
						settings.userfiles = _.without(settings.userfiles, url);
					});
