...
require(['lodash', 'diff', 'patch', 'chrome/utils', 'chrome/styles', 'chrome/globalSettings'], function(_, diff, patch, utils, styles, globalSettings) {
	...
	function enqueuePatch(collection, url, patches) {
		...
		var queuedItem = getPatchFromCollection(collection, url);
	...
	function getPatchFromCollection(collection, url) {
	...
	/**
	 * Runs CSSOM patching (for faster perceived performance) 
	 * operation on given CSS source
	 * @param  {String} file  File name of patched CSS source
	 * @param  {Array} patch  Patches to apply
	 */
	function runCSSOMPatching() {
		if (!cssomQueue.running && cssomQueue.length) {
			...
			chrome.devtools.inspectedWindow.eval(js, function(success) {
				cssomQueue.running = false;
				var patch = getPatchFromCollection(patchQueue, p.url);
				if (patch) {
					patch.debounce = success;
				}

				_runPatching();
				if (cssomQueue.length) {
					runCSSOMPatching();
				}
			});
		}
	...
	function handleWorkerMessage(data) {
		if (data.action == 'patch') {
			...
			if (data.success) {
				...
				styles.update(data.file, data.result.source, commitPatchedResource, data.result.options);
				sourcePatched(data);
			} else {
	...
	var requestUnsavedChanges = _.debounce(function() {
		utils.getPageSettings(function(settings) {
			if (!settings.assocs) {
				return;
			}

			styles.all(function(list) {
				var files = [];
				list.forEach(function(style) {
					if (settings.assocs[style.url]) {
						files.push(settings.assocs[style.url]);
					}
				});
	...
	styles.on('add', requestUnsavedChanges);
